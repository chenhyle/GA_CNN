...
class Net(nn.Module):
    def __init__(self):
        ...
        # self.fc1_complex=ComplexLinear(64 * 3 * 3, 128)
        # self.fc2_complex=ComplexLinear(96 + 128 * len(ahead_times), 64)
        # self.fc3_complex= ComplexLinear(64, 1)
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            ...
            deep_u = deep[:, 0, :, :, :, i]
            # print(deep_u.shape)# [128,4,31,31]
            ...
            deep_c1=self.att_block_C1(deep_c)*deep_c
            deep_c1 = self.complex_max_pool2d(self.complex_relu(self.complex_conv2(deep_c1)))
            deep_c1 = self.att_block_C2(deep_c1) * deep_c1

            # split 2
            deep_c=self.complex_max_pool2d(self.complex_relu(self.complex_conv2(deep_c)))
            deep_c2 = self.att_block_C2(deep_c) * deep_c
            #fuse
            time_seq1 = self.fuse_unit[i][0] / (self.fuse_unit[i][0] + self.fuse_unit[i][1]) * deep_c1 + \
                         self.fuse_unit[i][1] / (self.fuse_unit[i][0] + self.fuse_unit[i][1]) * deep_c2
            time_seq1 =self.complex_max_pool2d(self.complex_relu(self.complex_conv3(time_seq1)))
            time_seq1 =self.att_block_C3(time_seq1)*time_seq1
            #split 3
            deep_c = self.complex_max_pool2d(self.complex_relu(self.complex_conv3(deep_c)))
            deep_c3 = self.att_block_C3(deep_c) * deep_c
