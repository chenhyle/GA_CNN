...
class Net(nn.Module):
    def __init__(self):
        ...
        # self.clifford_conv4=CliffordConv2d(g=[-1,-1],in_channels=2,out_channels=2,kernel_size=1,padding=0)
        # self.clifford_conv1 = CliffordConv2d_DZ(g=[-1], in_channels=4, out_channels=32, kernel_size=1,padding=0)#64-32
        self.quaternion_conv_layer1 = QuaternionConv(
            in_channels=4,
            out_channels=32,
            kernel_size=3,
            stride=1,
        ...
        # self.clifford_conv2 = CliffordConv2d_DZ(g=[-1], in_channels=32, out_channels=64, kernel_size=1,padding=0)#减半
        self.quaternion_conv_layer2 = QuaternionConv(
            in_channels=32,
            out_channels=64,
            kernel_size=3,
            stride=1,
        ...
        # self.clifford_conv3 = CliffordConv2d_DZ(g=[-1], in_channels=64, out_channels=128, kernel_size=1,padding=0)
        self.quaternion_conv_layer3 = QuaternionConv(
            in_channels=64,
            out_channels=128,
            kernel_size=3,
            stride=1,
        ...
        # self.clifford_conv_new = CliffordConv2d_DZ(g=[-1], in_channels=128, out_channels=128, kernel_size=1, padding=0)
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            deep_u=deep[:,0,:,:,:,i]#[128,4,31,31]
            # print(deep_u.shape)
            deep_v=deep[:,1,:,:,:,i]#[128,4,31,31]

            #1、重新组织数据
            # 2、更改split 1
            # [96,32,15,15]
            deep_u_P=self.pool(F.relu(self.quaternion_conv_layer1(deep_u)))
            # [96,32,15,15]
            deep_u_P=self.att_block_1(deep_u_P)*deep_u_P
            # [96,32,15,15]
            deep_v_P = self.pool(F.relu(self.quaternion_conv_layer1(deep_v)))
            # [96,32,15,15]
            deep_v_P = self.att_block_1(deep_v_P) * deep_v_P
            #转换成复数，进行复数卷积
            # [96,32,15,15]虚数
            deep_uv = torch.complex(deep_u_P, deep_v_P)
            # [96,64,15,15]
            deep_uv_1 =self.complex_conv2(deep_uv)
            time_seq_1 = self.complex_relu(deep_uv_1)
            # [96,64,7,7]
            time_seq_1 = self.complex_max_pool2d(time_seq_1)
            #进行空间注意力机制
            #[96,64,7,7]
            time_seq_1 =self.att_block_C2(time_seq_1)*time_seq_1
            #3、更改split 2
            deep_u_P = self.pool(F.relu(self.quaternion_conv_layer2(deep_u_P)))  # [96,64,7,7]
            deep_u_P = self.att_block_2(deep_u_P) * deep_u_P  # [96,64,7,7]
            deep_v_P = self.pool(F.relu(self.quaternion_conv_layer2(deep_v_P)))  # [96,64,7,7]
            deep_v_P = self.att_block_2(deep_v_P) * deep_v_P  # [96,64,7,7]
            deep_uv_2 = torch.complex(deep_u_P, deep_v_P)#[96,64,7,7]

            # fuse 2
            # time_seq_2 = self.cross_unit[i][2]/(self.cross_unit[i][2]+self.cross_unit[i][3]) * deep_u_P +\
            # self.cross_unit[i][3]/(self.cross_unit[i][2]+self.cross_unit[i][3]) * deep_v_P
            time_seq_2 = self.fuse_unit[i][0]/(self.fuse_unit[i][0]+self.fuse_unit[i][1]) * time_seq_1+\
            self.fuse_unit[i][1]/(self.fuse_unit[i][0]+self.fuse_unit[i][1]) * deep_uv_2
            time_seq_2 =self.complex_conv3(time_seq_2)#128
            time_seq_2 =self.complex_relu(time_seq_2)
            time_seq_2 = self.complex_max_pool2d(time_seq_2)#[96,128,3,3]
            #[96,128,3,3]
            time_seq_2 = self.att_block_C3(time_seq_2) * time_seq_2


            #4、更改split 3
            deep_u_P = self.pool(F.relu(self.quaternion_conv_layer3(deep_u_P)))  # [96,128,3,3]
            deep_u_P = self.att_block_3(deep_u_P) * deep_u_P  # [96,128,3,3]#没改
            deep_v_P = self.pool(F.relu(self.quaternion_conv_layer3(deep_v_P)))  # [96,128,3,3]
            deep_v_P = self.att_block_3(deep_v_P) * deep_v_P  # [96,128,3,3]
            deep_uv_3 = torch.complex(deep_u_P, deep_v_P)
            #
            # fuse 3
            time_seq = self.fuse_unit[i][2]/(self.fuse_unit[i][2]+self.fuse_unit[i][3]) * time_seq_2 +\
            self.fuse_unit[i][3]/(self.fuse_unit[i][2]+self.fuse_unit[i][3]) * deep_uv_3
            # time_seq =self.complex_max_pool2d(time_seq)
            time_seq = self.att_block_C3(time_seq) * time_seq
            # print("time_seq.shape",time_seq.shape,time_seq[0],type(time_seq))#[128,128,3,3]

            # # print("time_seq.shape", time_seq.shape, time_seq[0])
            # print("self.quaternion_conv_layer1层参数", float(count_params(self.quaternion_conv_layer1)))
            # print("self.quaternion_conv_layer2层参数", float(count_params(self.quaternion_conv_layer2)))
            # print("self.quaternion_conv_layer3层参数", float(count_params(self.quaternion_conv_layer3)))
            # print("complex_conv2 层参数", float(count_params(self.complex_conv2)))
            # print("complex_conv3 层参数", float(count_params(self.complex_conv3)))
            # print("conv2 层参数", float(count_params(self.conv2)))
            # print("conv3 层参数", float(count_params(self.conv3)))
            # print("self.att_block_1层参数", float(count_params(self.att_block_1)))
            # print("self.att_block_2层参数", float(count_params(self.att_block_2)))
            # print("self.att_block_3层参数", float(count_params(self.att_block_3)))
            # print("self.att_block_C3层参数", float(count_params(self.att_block_C3)))
            ...
            # print("time_seq.shape", time_seq.shape)
            # time_seq =self.fc_new(time_seq)
            fc1_num_params = sum(p.numel() for p in self.fc1.parameters())
            # print("全连接层参数数量fc1:", fc1_num_params)
        ...
        # fc2_num_params = sum(p.numel() for p in self.fc2.parameters())
        # print("全连接层参数数量fc2:", fc2_num_params)
        # fc3_num_params = sum(p.numel() for p in self.fc3.parameters())
        # print("全连接层参数数量fc3:", fc3_num_params)
        # print('###############################################################')
