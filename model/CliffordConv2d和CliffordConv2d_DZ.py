...
train = pd.read_csv('D:/data/CMA_train_'+str(pre_seq*6)+'h.csv', header=None)
test= pd.read_csv('D:/data/CMA_test_'+str(pre_seq*6)+'h.csv', header=None)
...
for ahead_time in ahead_times:
    ...
    for pressure in pressures:
        ...
        train_reanalysis_csv = pd.read_csv('D:/data/ERA_Interim/'+folder+'/'+reanalysis_type+str(pressure)+'_train_31_31.csv', header=None)
        test_reanalysis_csv = pd.read_csv('D:/data/ERA_Interim/'+folder+'/'+reanalysis_type+str(pressure)+'_test_31_31.csv', header=None)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.cli_att_block_1 = nn.Sequential(
            CliffordConv2d_DZ(g=[-1], in_channels=32, out_channels=32, kernel_size=3, padding=1),
            CliffordBatchNorm2d([-1], channels=32),
        ...
        self.cli_att_block_2 = nn.Sequential(
            CliffordConv2d_DZ(g=[-1], in_channels=64, out_channels=64, kernel_size=3, padding=1),
            CliffordBatchNorm2d([-1], channels=64),
            nn.ReLU(inplace=True),
            # nn.LeakyReLU(negative_slope=0.01, inplace=True),
            CliffordConv2d_DZ(g=[-1], in_channels=64, out_channels=64, kernel_size=3, padding=1),
            CliffordBatchNorm2d([-1], channels=64),
        ...
        self.cli_att_block_3 = nn.Sequential(
            CliffordConv2d_DZ(g=[-1], in_channels=128, out_channels=128, kernel_size=3, padding=1),
        ...
        self.clifford_conv4 = CliffordConv2d(g=[-1, -1], in_channels=2, out_channels=2, kernel_size=3, padding=1)
        self.clifford_conv1 = CliffordConv2d_DZ(g=[-1], in_channels=4, out_channels=32, kernel_size=3, padding=1)  # 64-32
        self.pool = nn.MaxPool3d(kernel_size=(2, 2, 1), stride=(2, 2, 1))
        self.clifford_conv2 = CliffordConv2d_DZ(g=[-1], in_channels=32, out_channels=64, kernel_size=3, padding=1)  # 减半
        self.clifford_conv3 = CliffordConv2d_DZ(g=[-1], in_channels=64, out_channels=128, kernel_size=3, padding=1)
        # self.clifford_conv_new = CliffordConv2d_DZ(g=[-1], in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.fc1 = CliffordLinear(g=[-1], in_channels=128 * 3 * 3, out_channels=128)
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            # 1、对uv进行卷积操作
            deep_uv = deep[:, :, :, :, :, i]  # [96,2,4(P),31,31]
            deep_uv = deep_uv.permute(0, 1, 3, 4, 2)  # 把压强换到最后[96,2,31,31,4]//[64,2,31,31,4]
            deep_uv = F.relu(self.clifford_conv4(deep_uv))  # [96,2,31,31,4]对uv进行卷积
            deep_uv = self.dropout(deep_uv)
            deep_P = deep_uv.permute(0, 4, 2, 3, 1)  # 96,4,31,31,2
            # 2、split 1
            # 对压强层进行卷积 激活 最大池化，但是由于
            deep_P = self.pool(F.relu(self.clifford_conv1(deep_P)))  # (96,32,15,15,2)
            # print(deep_P.shape)
            deep_P = self.cli_att_block_1(deep_P) * deep_P  # 96,32,15,15,2
            time_seq_1 = self.pool(F.relu(self.clifford_conv2(deep_P)))  # 96,64,7,7,2
            time_seq_1 = self.att_block_1(time_seq_1) * time_seq_1  # 96,64,7,7,2

            # 3、更改split 2
            deep_P = self.pool(F.relu(self.clifford_conv2(deep_P)))  # 96,64,7,7,2
            deep_P = self.cli_att_block_2(deep_P) * deep_P  # 96,64,7,7,2
            time_seq_2 = self.fuse_unit[i][0] / (self.fuse_unit[i][0] + self.fuse_unit[i][1]) * time_seq_1 + \
                         self.fuse_unit[i][1] / (self.fuse_unit[i][0] + self.fuse_unit[i][1]) * deep_P  # 96,64,7,7,2
            time_seq_2 = self.pool(F.relu(self.clifford_conv3(time_seq_2)))  # 96,128,3,3,2
            time_seq_2 = self.att_block_3(time_seq_2) * time_seq_2  # 96,128,3,3,2

            # 4、更改split 3
            deep_P = self.pool(F.relu(self.clifford_conv3(deep_P)))  # 96,128,3,3,2
            deep_P = self.cli_att_block_3(deep_P) * deep_P  # 96,128,3,3,2

            # 5、fuse 3
            time_seq = self.fuse_unit[i][2] / (self.fuse_unit[i][2] + self.fuse_unit[i][3]) * time_seq_2 + \
                       self.fuse_unit[i][3] / (self.fuse_unit[i][2] + self.fuse_unit[i][3]) * deep_P
            time_seq = self.att_block_3(time_seq) * time_seq  # 96,128,3,3,2
            time_seq = time_seq.view(-1, 128 * 3 * 3, 2)
            # print("time_seq", time_seq.shape)
            time_seq = self.fc1(time_seq)
            deep_u = time_seq[:, :, 0]  # 96,128,3,3
            deep_v = time_seq[:, :, 1]
        ...
        #     # print("time_seq", time_seq.shape)
        #     time_seq = self.fc1(time_seq)
        #     # print("time_seq", time_seq.shape)  # 96,128
