...
class Net(nn.Module):
    def __init__(self):
        ...
        self.att_block_3 = nn.Sequential(
            QuaternionConv(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1),
            # nn.BatchNorm2d(64),
            QBatchNorm(256),
            nn.ReLU(inplace=True),
            QuaternionConv(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1),
        ...
        # deep
        self.qn_conv1 = QuaternionConv(in_channels=4, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(2, 2)
        self.qn_conv2 = QuaternionConv(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1)
        self.qn_conv3 = QuaternionConv(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1)
        self.qn_conv2_fuse = QuaternionConv(in_channels=64, out_channels=128, kernel_size=3, stride=1, padding=1)
        self.qn_conv3_fuse = QuaternionConv(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1)
        # self.fc1 = nn.Linear(64 * 3 * 3, 64)
        # self.fc2 = nn.Linear(96 + 64 * len(ahead_times), 64)
        self.fc1 = QuaternionLinearAutograd(256 * 3 * 3, 128)
