...
for ahead_time in ahead_times:
    ...
    for pressure in pressures:
        ...
        train_reanalysis_csv = pd.read_csv(
            'D:/data/ERA_Interim/' + folder + '/' + reanalysis_type + str(pressure) + '_train_31_31.csv',
            header=None)
        test_reanalysis_csv = pd.read_csv(
            'D:/data/ERA_Interim/' + folder + '/' + reanalysis_type + str(pressure) + '_test_31_31.csv',
            header=None)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.att_block_2_fuse = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=1),
        ...
        self.att_block_3_fuse = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, padding=1),
        ...
        # cross
        self.cross_unit = nn.Parameter(data=torch.ones(len(ahead_times), 6))  # 4*6
        # fuse
        self.fuse_unit = nn.Parameter(data=torch.ones(len(ahead_times), 4))  # 4*4

        # deep
        self.conv1 = nn.Conv2d(4, 64, kernel_size=3, padding=(1, 1))
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, padding=(1, 1))
        self.conv_fuse1 = nn.Conv2d(64, 128, kernel_size=3, padding=(1, 1))
        self.conv3 = nn.Conv2d(128, 256, kernel_size=3, padding=(1, 1))
        self.conv_fuse2 = nn.Conv2d(128, 256, kernel_size=3, padding=(1, 1))
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            ...
            time_seq_1 = self.pool(F.relu(self.conv_fuse1(time_seq_1)))
            time_seq_1 = self.att_block_2_fuse(time_seq_1) * time_seq_1
            ...
            time_seq_2 = self.pool(F.relu(self.conv_fuse2(time_seq_2)))
            time_seq_2 = self.att_block_3_fuse(time_seq_2) * time_seq_2
            ...
            time_seq = self.att_block_3_fuse(time_seq) * time_seq

            time_seq = time_seq.view(-1, 256 * 3 * 3)
            time_seq = self.fc1(time_seq)
            seq_list.append(time_seq)
        wide = wide.view(-1, 96)
        wide_n_deep = torch.cat((wide, seq_list[0]), 1)  # 128,224
...
model_name = 'TIP_cnn_3_2_fuse.pkl'
...
log_name = 'TIP_cnn_3_2fuse'
logging.basicConfig(filename=log_name, level=logging.INFO)
logging.info(f"原模型 数据集拿到外面分")
logging.info(f"原模型 空间注意力kernels 3*3  卷积分开定义")
...
#         print('epochs [%d/%d] train_loss: %.5f val_loss: %.5f' % (
#         epoch + 1, epochs, total_train_loss / train_count, total_val_loss / val_count))
#         logging.info(
#             f"Epoch {epoch + 1}/{epochs}; train_loss:{total_train_loss / train_count:.5f};val_loss:{total_val_loss / val_count:.5f}")
